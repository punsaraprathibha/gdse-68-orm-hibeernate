...
public class DetachedState {
    public static void main(String[] args) {
        ...
        if (isPersistent) {
            ...
            System.out.println("This object is " +
                    "not in Persistent state");
        }
        session.close();

        // Detached State
        Session detatchSession = SessionFactoryConfig.getInstance().getSession();
        detatchSession.detach(customer);
        boolean isPersists = detatchSession.contains(customer);
        if (isPersists) {
            System.out.println("This object is " +
                    "in Persistent state");
        } else {
        ...
        // Persistent State
        customer.setAddress("Hambantota");
        detatchSession.save(customer);

        boolean isPersistent1 = detatchSession.contains(customer);
        if (isPersistent1) {
            System.out.println("This object is " +
                    "in Persistent state");
        } else {
